...
const Inbox = ({
  ...
  const [newConvoPanel, setNewConvoPanel] = React.useState(false); // load convos
    ...
    if (response.data && response.data.response_data && response.errors.length <= 0) {
      switch (response.data.request) {
        ...
        case 'CreateNewConvo':
          ...
          ClearTextbox(newConvoMessageInput, false, true); // Success Message
  ...
  return (
    ...
    }, "Leave Conversation")), selectedConvoData['owner_id'] === selectedConvoData['self'] && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "inbox_convo_details_action"
