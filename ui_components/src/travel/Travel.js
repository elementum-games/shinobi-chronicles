...
const Travel = ({
    ...
    return (
        <>
        <div className='contentDivMessages'>
            {/* ERROR MESSAGE */}
            { (errorMessage) && (<p className='systemMessage-new systemMessage-new-error'>{errorMessage}</p>)}
        </div>
            <div className='travelAction'>
                {(mapData && mapData.portal_display) && (
                    <button onClick={() => EnterPortal(mapData.portal_id)}>{mapData.portal_text}</button>
                )}
                {(mapData && mapData.mission_button) && (
                    <a href={missionLink}><button>Go To Mission Location</button></a>
                )}
            </div>
        <div className='travel'>
            <div className='travelMapContainer'>
                {(mapData) && (
                    <Map map_data={ mapData }
                         onClick={MovePlayer} />
                )}
                <div className='travelLabel'>
                    {feedback}
                </div>
            </div>
            {(scoutData) && (
