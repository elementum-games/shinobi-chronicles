...
class UserDailyTasks {
    ...
    public function __construct($system, $user_id, $user_rank_num, $total_skills, $total_attributes, $pvp_rep) {
        $this->system = $system;
    ...
    public function generateNewTasks($update_db = true): void {
        $this->tasks = DailyTask::generateNewTasks($this->user_rank_num, $this->total_skills, $this->total_attributes, $this->pvp_rep);
        if($update_db) {
            $this->update(true);
        }
    }
