...
function modPanel() {
	...
	global $self_link;
	// Staff level check
	if(!isset($SC_MODERATOR) or $player->staff_level < $SC_MODERATOR) {
		return false;
	}
	// $page = $_GET['page'];
	$display_menu = true;
	...
	if($_POST['ban']) {
		try {
			...
			if(!empty($user_data['ban_type']) && $player->staff_level == $SC_MODERATOR) {
	...
	// View record
		...
		try {
			...
			if($staff_level >= $player->staff_level && $player->staff_level < $SC_HEAD_ADMINISTRATOR) {
			...
			foreach($reports as $id => $report) {
				echo "<tr>
					...
					<td><a href='$report_link&page=view_report&report_id=$id'>View</a></td>
				</tr>";
			}
			echo "</table>";
			$display_menu = false;	
	...
	// Banned user list
	// Locked out users
	if($_GET['unlock_account'] && $player->staff_level >= $SC_HEAD_MODERATOR) {
	...
	if($_GET['view'] == 'banned_users') {
		try {
			...
			while($user = $system->db_fetch($result)) {
				echo "<tr>
					<td><a href='$members_link&user={$user['user_name']}'>" . $user['user_name'] . "</a></td>
					<td>";
					$add_comma = false;
					if($user['ban_type']) {
