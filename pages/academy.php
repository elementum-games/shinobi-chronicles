...
function academy() {
    ...
    if (isset($_POST['confirm_lesson'])) {
        try {
			...
			$player->currency->subtractMoney($lesson_cost, "Paid " . $lesson_cost . Currency::MONEY_SYMBOL ." for lessons.");

            // Add yen to sensei
            $sensei_player = User::loadFromId($system, $lesson_sensei['sensei_id']);
            $sensei_player->loadData(User::UPDATE_NOTHING);
            $sensei_player->currency->addMoney($lesson_cost / 5, "Earned " . $lesson_cost / 5 . Currency::MONEY_SYMBOL . " for lessons.");
