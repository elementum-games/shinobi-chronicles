...
function marriage() {
    ...
    if(isset($_POST['confirm_divorce'])) {
        try {
            ...
            if ($system->db_last_num_rows) {
                // Current Marriage = Spouse of Player
                ...
                else
                {
                    ...
                    // Player's spouse is mixed up. Both players are not married to each other. Clear player's spouse marriage chain.
                    if (!(intval($players_spouse['user_id']) === intval($spouses_spouse['spouse']))) {
                        // Clear the marriage of the player's spouse.
                        $system->query("UPDATE `users` SET `spouse`='0', `marriage_time`='0' WHERE `user_id`='$player->spouse' OR `user_id`=" . $players_spouse['spouse'] . " LIMIT 2");
                        if (!$system->db_last_affected_rows) {
                            throw new Exception("Error processing divorce!");
                        }
                    }
