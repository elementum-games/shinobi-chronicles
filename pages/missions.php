...
function runActiveMission(): bool {
    ...
    if($mission->mission_type === Mission::TYPE_SURVIVAL && !empty($_GET['retreat'])) {
        $player->battle_id = 0;
        $player->mission_stage['stage_id'] = 4;
    }
    else if($player->mission_stage['action_type'] == 'travel' or $player->mission_stage['action_type'] == 'search') {
        if($player->location == $player->mission_stage['action_data']) {
            $player->mission_stage += 1;
        }
    }
